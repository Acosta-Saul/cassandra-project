async function getData() {
  ...
  if (data.length > 0) {
    ...
    // Crea la tabla en el HTML (No se va a ver en el index.html, solo se muestra en la página web, pueden ver la tabla dando click derecho en la página, inspeccionar, revisan body, container, div.data-container, ahí está la tabla que se está creando aquí)
    ...
    data.forEach(item => {
      ...
      // Campos que quiero mostrar en la tabla, item[campo] va a extraer el contenido de la tabla, ejemplo item[nombre], me dará el nombre del registro y lo almacenará en el td.textContent, luego ese td se guarda como hijo del row con el appendChild (Esto es cosa de JavaScript)
      campos.forEach(campo => {
        if (campo !== 'id') { // Evitar mostrar la columna de ID en la tabla
